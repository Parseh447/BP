...
const generateRemark = (index, port) => {
    ...
    switch (index) {
        case 0:
        case 1:
            remark = `ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL - Domain_${index + 1} : ${port}`;
            break;
        case 2:
        case 3:
            remark = `ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL - IPv4_${index - 1} : ${port}`;
            break;
        case 4:
        case 5:
            remark = `ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL - IPv6_${index - 3} : ${port}`;
            break;
        default:
            remark = `ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL - Clean IP_${index - 5} : ${port}`;
            break;
    }
...
const buildWorkerLessConfig = async (env, client) => {
    ...
    fragConfig.remarks  = 'ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL Frag - WorkerLess â­'
...
const getFragmentConfigs = async (env, hostName, client) => {
    ...
    bestPing.remarks = 'ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL Frag - Best Ping ðŸ’¥';
    ...
    bestFragment.remarks = 'ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL Frag - Best Fragment ðŸ˜Ž';
...
const getWarpConfigs = async (env, client) => {
    ...
    singboxWarpConfig.outbounds[0].outbounds = ['â­ Warp Best Ping ðŸš€'];
    singboxWarpConfig.outbounds[1].tag = 'â­ Warp Best Ping ðŸš€';
    ...
    xrayWarpBestPing.remarks = 'ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL - Warp Best Ping ðŸš€'
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: `ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL - Warp ${index + 1} ðŸ‡®ðŸ‡·`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = `ðŸ’¥ ðŸ‡¸ðŸ‡¬ TUNEL - WoW ${index/2 + 1} ðŸŒ`;
...
const buildWoWOutbounds = async (env, remoteDNS, localDNS, blockAds, bypassIran, blockPorn, bypassLAN, wowEndpoint) => {
    ...
    wowEndpoint.split(',').forEach( (endpoint, index) => {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            singboxOutbound.tag = i === 1 ? `warp-ir_${index + 1}` : `â­ WoW ${index + 1} ðŸŒ`;    
...
const renderHomePage = async (env, hostName, fragConfigs) => {
            ...
            <tr>
                <td>
                    ${config.address === 'Best-Ping' 
                        ? `<div  style="justify-content: center;"><span><b>â­ Best-Ping ðŸ’¥</b></span></div>` 
                        : config.address === 'WorkerLess'
                            ? `<div  style="justify-content: center;"><span><b>â­ WorkerLess â­</b></span></div>`
                            : config.address === 'Best-Fragment'
                                ? `<div  style="justify-content: center;"><span><b>â­ Best-Fragment ðŸ˜Ž</b></span></div>`
                                : config.address
                    }
                </td>
                <td>
	...
	<body>
		<h1>Control panel <span style="font-size: smaller;">${panelVersion}</span> ðŸ’¦</h1>
		<div class="form-container">
			...
			<form id="configForm">
                ...
                <div class="form-control">
                    <label>ðŸ”Ž Endpoint Scanner</label>
        ...
        const getWarpConfigs = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    alert('Warp configs updated successfully! ðŸ˜Ž');
                } else {
...
const xrayWgOutboundTemp = {
    ...
    settings: {
        ...
        peers: [
            {
                endpoint: ".cloudflareclient.com:2408",
                publicKey: ""
            }
        ],
        reserved: [],
